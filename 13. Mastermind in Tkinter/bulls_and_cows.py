...
sense. 3 blacks and 1 white make no sense for example. """
    ...
    if (rightly_positioned + permutated > number_of_positions) or (rightly_positioned + permutated < len(colours) - number_of_positions):
        return False
...
def check(p1, p2):
    """ check() calcualtes the number of bulls (blacks) and cows
(whites) of two permutations """
    blacks = 0
    whites = 0
